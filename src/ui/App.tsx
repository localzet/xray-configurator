          ...
          <Stack gap="md">
            <Group className="responsive-header" justify="space-between" align="flex-start">
              ...
              <Group className="responsive-controls">
                ...
                />
                <Switch checked={advancedMode} label={t.advancedMode} onChange={(event) => setAdvancedMode(event.currentTarget.checked)} />
              </Group>
            </Group>
            <Box className="presets-grid">
              {presets.map((preset) => (
                <Tooltip key={preset.id} label={language === 'ru' ? preset.descriptionRu : preset.description}>
                  <Button leftSection={<IconWand size={16} />} variant="light" onClick={() => syncConfig(preset.create())}>
          ...
          <Tabs.Panel value="builder">
            <Box className="builder-layout">
              ...
              <Box className="builder-main">
                <Stack gap="lg">
                  <GlobalSections config={config} language={language} query={query} advancedMode={advancedMode} onChange={syncConfig} />
                  <InboundList config={config} language={language} query={query} advancedMode={advancedMode} onChange={syncConfig} />
                  <OutboundList config={config} language={language} query={query} advancedMode={advancedMode} onChange={syncConfig} />
                </Stack>
              </Box>
                  ...
                  <Group className="responsive-header" justify="space-between" mb="sm">
                    ...
                    <Group gap="xs">
                      <CopyButton value={output}>
                        {({ copied, copy }) => (
                          <Button size="xs" variant="light" leftSection={copied ? <IconCircleCheck size={14} /> : <IconCopy size={14} />} onClick={copy}>
                            {t.copy}
                          </Button>
                        )}
                      </CopyButton>
                      <Button size="xs" variant="subtle" color="red" leftSection={<IconRefresh size={14} />} onClick={() => syncConfig(defaultConfig())}>
                        {t.reset}
                      </Button>
                    </Group>
                  </Group>
  ...
  return (
    <Paper className="panel" p="md" radius="md">
      <Stack gap="md">
        ...
        />
        <Switch checked={advancedMode} label={t.advancedMode} onChange={(event) => onAdvancedModeChange(event.currentTarget.checked)} />
        <Divider />
        <Stack gap="xs">
          {presets.map((preset) => (
            <Tooltip key={preset.id} label={language === 'ru' ? preset.descriptionRu : preset.description}>
              <Button fullWidth leftSection={<IconWand size={16} />} variant="light" onClick={() => onPresetLoad(preset.create())}>
                {language === 'ru' ? preset.labelRu : preset.label}
              </Button>
            </Tooltip>
          ))}
          ...
          {selected ? (
            <Stack>
              ...
              <Group gap={5}>
                ...
                {selected.transports.map((tag) => <Badge key={tag} color="teal" variant="light">{tag}</Badge>)}
                {selected.securities.map((tag) => <Badge key={tag} color="violet" variant="light">{tag}</Badge>)}
                {selected.features.map((tag) => <Badge key={tag} color="gray" variant="light">{tag}</Badge>)}
              </Group>
...
function OutboundList({
  ...
  const add = () => {
    ...
    onChange({
      ...config,
      outbounds: [...outbounds, { tag: `outbound-${outbounds.length + 1}`, protocol: protocol.value as OutboundConfig['protocol'], settings: protocol.defaultSettings }],
    });
  };
...
function EndpointCard({
  ...
  const changeProtocol = (value: string | null) => {
    ...
    updateBase({
      ...
      streamSettings: nextSupportsStream ? (endpoint.streamSettings ?? { network: 'tcp', security: 'none' }) : undefined,
    });
  };
      ...
      <Stack>
        ...
        <Grid>
          <Grid.Col span={{ base: 12, sm: 4 }}>
            <TextInput label={t.tag} value={endpoint.tag} onChange={(event) => updateBase({ tag: event.currentTarget.value })} />
          </Grid.Col>
          {mode === 'inbound' && (
            <>
              <Grid.Col span={{ base: 12, sm: 4 }}>
                <TextInput label={t.listen} value={(endpoint as InboundConfig).listen ?? ''} onChange={(event) => updateBase({ listen: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, sm: 4 }}>
                <TextInput
                  label={t.port}
        ...
        {mode === 'inbound' && (
          <>
            ...
            <SniffingEditor endpoint={endpoint as InboundConfig} language={language} onChange={(next) => onChange(next)} />
          </>
        )}
      ...
      {security === 'tls' && (
        <>
          ...
          <CertificatesEditor
            ...
            language={language}
            onChange={(certificates) => setNested('tlsSettings', { ...((stream.tlsSettings ?? {}) as RecordValue), certificates })}
          />
          <FieldGrid
...
function SniffingEditor({ endpoint, language, onChange }: { endpoint: InboundConfig; language: Language; onChange: (endpoint: InboundConfig) => void }) {
  ...
  return (
    <Grid>
      <Grid.Col span={{ base: 12, sm: 3 }}>
        <Switch checked={enabled} label={labels[language].enabled} onChange={(event) => update({ enabled: event.currentTarget.checked })} />
      </Grid.Col>
      {enabled && (
        <>
          ...
          <Grid.Col span={{ base: 12, sm: 2 }}>
            <Switch checked={Boolean(sniffing.metadataOnly)} label="metadataOnly" onChange={(event) => update({ metadataOnly: event.currentTarget.checked })} />
          </Grid.Col>
          <Grid.Col span={{ base: 12, sm: 2 }}>
            <Switch checked={Boolean(sniffing.routeOnly)} label="routeOnly" onChange={(event) => update({ routeOnly: event.currentTarget.checked })} />
          </Grid.Col>
        </>
      )}
    </Grid>
          ...
          <Paper className="nested-row" key={index} p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
            <Grid align="end">
              {clientUsesId(protocol) && (
                <Grid.Col span={{ base: 12, md: 4 }}>
                  <TextInput
                    ...
                    rightSection={
                      <ActionIcon variant="subtle" onClick={() => updateClient(index, { id: crypto.randomUUID() })}>
                        <IconKey size={16} />
                      </ActionIcon>
                    }
              ...
              {clientUsesPassword(protocol) && (
                <Grid.Col span={{ base: 12, md: 4 }}>
                  <PasswordInput
                    ...
                    rightSection={
                      <ActionIcon variant="subtle" onClick={() => updateClient(index, { [protocol === 'hysteria' ? 'auth' : 'password']: randomToken(24) })}>
                        <IconKey size={16} />
                      </ActionIcon>
                    }
              ...
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="email" value={String(client.email ?? '')} onChange={(event) => updateClient(index, { email: event.currentTarget.value })} />
              </Grid.Col>
              ...
              <Grid.Col span={{ base: 12, md: 2 }}>
                <NumberInput label="level" allowDecimal={false} value={typeof client.level === 'number' ? client.level : undefined} onChange={(value) => updateClient(index, { level: value === '' ? 0 : Number(value) })} />
              </Grid.Col>
              <Grid.Col className="inline-action-col" span={{ base: 12, md: 1 }}>
                <ActionIcon color="red" variant="subtle" onClick={() => onChange(clients.filter((_, clientIndex) => clientIndex !== index))}>
                  <IconTrash size={18} />
                </ActionIcon>
              </Grid.Col>
            </Grid>
...
function DnsServersEditor({ servers, language, onChange }: { servers: JsonValue[]; language: Language; onChange: (servers: JsonValue[]) => void }) {
  const t = labels[language];
  ...
  return (
    <Paper p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
      <Group justify="space-between" mb="sm">
        ...
        <Button size="xs" variant="light" leftSection={<IconPlus size={14} />} onClick={() => onChange([...servers, '1.1.1.1'])}>
          {t.addServer}
        </Button>
      </Group>
      <Stack gap="sm">
            ...
            <Paper className="nested-row" key={index} p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
              <Grid align="end">
                ...
                {objectMode ? (
                  <>
                    <Grid.Col span={{ base: 12, md: 3 }}>
                      <TextInput label="address" value={String(record.address ?? '')} onChange={(event) => updateServer(index, { ...record, address: event.currentTarget.value })} />
                    </Grid.Col>
                    <Grid.Col span={{ base: 12, md: 2 }}>
                      <NumberInput
                        label="port"
                    ...
                    <Grid.Col span={{ base: 12, md: 3 }}>
                      <Select
                        ...
                        onChange={(queryStrategy) => updateServer(index, { ...record, queryStrategy: queryStrategy ?? '' })}
                      />
                    </Grid.Col>
                    <Grid.Col span={{ base: 12, md: 2 }}>
                      <Switch checked={Boolean(record.skipFallback)} label="skipFallback" onChange={(event) => updateServer(index, { ...record, skipFallback: event.currentTarget.checked })} />
                    </Grid.Col>
                  </>
                ) : (
                  <Grid.Col span={{ base: 12, md: 8 }}>
                    <TextInput label="server" value={String(server ?? '')} onChange={(event) => updateServer(index, event.currentTarget.value)} />
                  </Grid.Col>
                )}
                <Grid.Col className="inline-action-col" span={{ base: 12, md: 1 }}>
                  <ActionIcon color="red" variant="subtle" onClick={() => onChange(servers.filter((_, serverIndex) => serverIndex !== index))}>
                    <IconTrash size={18} />
                  </ActionIcon>
                </Grid.Col>
              </Grid>
...
function FallbacksEditor({ fallbacks, language, onChange }: { fallbacks: RecordValue[]; language: Language; onChange: (fallbacks: RecordValue[]) => void }) {
  ...
  return (
    <Paper p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
      <Group className="responsive-header" justify="space-between" mb="sm">
        ...
        <Button size="xs" variant="light" leftSection={<IconPlus size={14} />} onClick={() => onChange([...fallbacks, { dest: 80 }])}>
          {addLabel}
        </Button>
      </Group>
      <Stack gap="sm">
        {fallbacks.map((fallback, index) => (
          <Paper className="nested-row" key={index} p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
            <Grid align="end">
              <Grid.Col span={{ base: 12, md: 2 }}>
                <TextInput label="name" value={String(fallback.name ?? '')} onChange={(event) => updateFallback(index, { name: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 2 }}>
                <TextInput label="alpn" value={String(fallback.alpn ?? '')} onChange={(event) => updateFallback(index, { alpn: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="path" value={String(fallback.path ?? '')} onChange={(event) => updateFallback(index, { path: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 2 }}>
                <Select
                  clearable
              ...
              <Grid.Col span={{ base: 12, md: 1 }}>
                <NumberInput label="xver" allowDecimal={false} value={typeof fallback.xver === 'number' ? fallback.xver : undefined} onChange={(value) => updateFallback(index, { xver: value === '' ? 0 : Number(value) })} />
              </Grid.Col>
              <Grid.Col className="inline-action-col" span={{ base: 12, md: 1 }}>
                <ActionIcon color="red" variant="subtle" onClick={() => onChange(fallbacks.filter((_, fallbackIndex) => fallbackIndex !== index))}>
                  <IconTrash size={18} />
                </ActionIcon>
              </Grid.Col>
            </Grid>
...
function CertificatesEditor({ certificates, language, onChange }: { certificates: RecordValue[]; language: Language; onChange: (certificates: RecordValue[]) => void }) {
    ...
    <Paper p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
      <Group className="responsive-header" justify="space-between" mb="sm">
        ...
        <Button
          ...
          onClick={() => onChange([...certificates, { certificateFile: '/etc/ssl/fullchain.pem', keyFile: '/etc/ssl/privkey.pem' }])}
        >
          {addLabel}
        </Button>
      </Group>
        ...
        {certificates.map((certificate, index) => (
          <Paper className="nested-row" key={index} p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
            <Grid align="end">
              ...
              <Grid.Col span={{ base: 12, md: 5 }}>
                <TextInput label="keyFile" value={String(certificate.keyFile ?? '')} onChange={(event) => updateCertificate(index, { keyFile: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 2 }}>
                <Select
                  clearable
              ...
              <Grid.Col span={{ base: 12, md: 5 }}>
                <PasswordInput label="key" value={String(certificate.key ?? '')} onChange={(event) => updateCertificate(index, { key: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col className="inline-action-col" span={{ base: 12, md: 1 }}>
                <ActionIcon color="red" variant="subtle" onClick={() => onChange(certificates.filter((_, certificateIndex) => certificateIndex !== index))}>
                  <IconTrash size={18} />
                </ActionIcon>
              </Grid.Col>
            </Grid>
  ...
  return (
    <Paper p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
      <Group justify="space-between" mb="sm">
        ...
        <Button size="xs" variant="light" leftSection={<IconPlus size={14} />} onClick={() => onChange([...rules, { type: 'field', outboundTag: outboundTags[0] ?? 'direct' }])}>
          {t.addRule}
        </Button>
      </Group>
      <Stack gap="sm">
        {rules.map((rule, index) => (
          <Paper className="nested-row" key={index} p="sm" radius="md" withBorder bg="rgba(255,255,255,0.02)">
            <Grid align="end">
              <Grid.Col span={{ base: 12, md: 2 }}>
                <Select label="type" data={['field']} value={String(rule.type ?? 'field')} onChange={(type) => updateRule(index, { type: type ?? 'field' })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <Select
                  clearable
              ...
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="port" value={String(rule.port ?? '')} onChange={(event) => updateRule(index, { port: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <MultiSelect
                  searchable
              ...
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="sourcePort" value={String(rule.sourcePort ?? '')} onChange={(event) => updateRule(index, { sourcePort: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TagsInput label="user" value={Array.isArray(rule.user) ? rule.user.map(String) : []} onChange={(user) => updateRule(index, { user })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TagsInput label="localIP" value={Array.isArray(rule.localIP) ? rule.localIP.map(String) : []} onChange={(localIP) => updateRule(index, { localIP })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="localPort" value={String(rule.localPort ?? '')} onChange={(event) => updateRule(index, { localPort: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TagsInput label="process" value={Array.isArray(rule.process) ? rule.process.map(String) : []} onChange={(process) => updateRule(index, { process })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="ruleTag" value={String(rule.ruleTag ?? '')} onChange={(event) => updateRule(index, { ruleTag: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 3 }}>
                <TextInput label="balancerTag" value={String(rule.balancerTag ?? '')} onChange={(event) => updateRule(index, { balancerTag: event.currentTarget.value })} />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 6 }}>
                <JsonInput
                  autosize
              ...
              <Grid.Col className="inline-action-col" span={{ base: 12, md: 1 }}>
                <ActionIcon color="red" variant="subtle" onClick={() => onChange(rules.filter((_, ruleIndex) => ruleIndex !== index))}>
                  <IconTrash size={18} />
                </ActionIcon>
              </Grid.Col>
            </Grid>
...
function SectionEditor({
  ...
  return (
    <Stack>
      ...
      <FieldGrid fields={fields} language={language} query={query} advancedMode={advancedMode} value={value} onChange={onChange} />
    </Stack>
  );
}
...
function ProtocolSettingsEditor({
  ...
  return (
    <Stack>
      ...
      <FieldGrid fields={fields} language={language} query={query} advancedMode={advancedMode} value={endpoint.settings} onChange={onChange} />
    </Stack>
  );
}
  ...
  return (
    <Grid>
      {visibleFields.map((field) => (
        <Grid.Col key={field.key} span={field.kind === 'json' || field.kind === 'textarea' ? 12 : { base: 12, sm: 6 }}>
          <FieldInput definition={field} language={language} value={value[field.key]} onChange={(next) => update(field.key, next)} />
        </Grid.Col>
      ))}
    </Grid>
  );
...
function FieldInput({
  ...
  if (definition.kind === 'switch') {
    return <Switch checked={Boolean(value)} label={label} onChange={(event) => onChange(event.currentTarget.checked)} />;
  }

  if (definition.kind === 'select') {
    return <Select clearable label={label} data={definition.options ?? []} value={value == null ? null : String(value)} onChange={(next) => onChange(next ?? undefined)} />;
  }

  if (definition.kind === 'number') {
    return <NumberInput allowDecimal={false} label={label} value={typeof value === 'number' ? value : undefined} onChange={(next) => onChange(next === '' ? undefined : Number(next))} />;
  }
  ...
  if (definition.kind === 'textarea') {
    return <Textarea autosize label={label} minRows={3} value={String(value ?? '')} onChange={(event) => onChange(event.currentTarget.value)} />;
  }

  if (definition.kind === 'password') {
    return <PasswordInput label={label} rightSection={rightSection} value={String(value ?? '')} onChange={(event) => onChange(event.currentTarget.value)} />;
  }
...
function defaultSection(section: string): RecordValue {
  switch (section) {
    ...
    case 'burstObservatory':
      return { subjectSelector: ['proxy'], pingConfig: { destination: 'https://www.gstatic.com/generate_204', interval: '1m' } };
    case 'geodata':
