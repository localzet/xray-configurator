...
export type TransportProtocol = 'raw' | 'tcp' | 'ws' | 'grpc' | 'xhttp' | 'splithttp' | 'kcp' | 'httpupgrade' | 'hysteria';
...
export const inboundProtocols: ProtocolDefinition[] = [
  {
    ...
    defaultSettings: { clients: [{ id: crypto.randomUUID(), flow: 'xtls-rprx-vision', email: 'user@example.com' }], decryption: 'none' },
    fields: [
...
export const outboundProtocols: ProtocolDefinition[] = [
  ...
  },
  {
    ...
    defaultSettings: { vnext: [{ address: 'example.com', port: 443, users: [{ id: crypto.randomUUID(), encryption: 'none', flow: 'xtls-rprx-vision' }] }] },
  ...
  },
  {
    ...
    defaultSettings: { servers: [{ address: 'example.com', port: 8388, method: '2022-blake3-aes-128-gcm', password: randomToken(24) }] },
